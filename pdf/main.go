...
func main() {
	lineItems := []LineItem{
		{
			...
			UnitName:       "Some long description with lots and lots of lines to test what's wrong with the wrapping.",
	...
	// Banner - Logo
	pdf.ImageOptions("images/jump.gif", 248.0, 10, 0, bannerHt/1.5, false, gofpdf.ImageOptions{
		ReadDpi: true,
	}, 0, "")
	...
	x = w - xIndent - 2.0 - 120.0
	pdf.MoveTo(x, y)
	pdf.CellFormat(120.0, lineHt, "Amount", gofpdf.BorderNone, gofpdf.LineBreakNone, gofpdf.AlignRight, false, 0, "")
...
func trailerLine(pdf *gofpdf.Fpdf, x, y float64, label string, amount int) (float64, float64) {
	...
	pdf.CellFormat(80.0, lineHt, toUSD(amount), gofpdf.BorderNone, gofpdf.LineBreakNone, gofpdf.AlignRight, false, 0, "")
	y = y + lineHt*1.5
	return origX, y
}
...
func lineItem(pdf *gofpdf.Fpdf, x, y float64, lineItem LineItem) (float64, float64) {
	...
	x = w - xIndent - 2.0 - 120.0
	pdf.MoveTo(x, y)
	pdf.CellFormat(120.0, lineHt, toUSD(lineItem.PricePerUnit*lineItem.UnitsPurchased), gofpdf.BorderNone, gofpdf.LineBreakNone, gofpdf.AlignRight, false, 0, "")
	if maxY > y {
		y = maxY
	}
	y = y + lineHt*1.75
